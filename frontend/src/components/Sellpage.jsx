...
const Sellpage = () => {
  ...
  // Add coupon Function
    const addCouponFunction = async (e) => {
        if (coupon.validity === "" || coupon.price === "" || coupon.offer === "" || coupon.minimumAmt === "" || coupon.code === "") {
    ...
    <div>
      ...
      {loading && <Loader/>}
        <form onSubmit={addCouponFunction} className="bg-gradient-to-tl from-cyan-500 to-green-500  h-full max-w-96 p-6 rounded-lg shadow-inner-outer">
          ...
          <button  type="submit" className="w-full mt-3 mb-4 h-10 text-center font-semibold text-white bg-blue-500 rounded-md shadow-sm hover:bg-blue-700   ">
            Upload
          </button>
        </form>
      </div>
